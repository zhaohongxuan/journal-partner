...
- **第二语言中文**：切换到中文后，设置页、快速记录侧边栏、统计页、Notice 提示全部变为中文
...
把微信当作 Journal Partner 的远程输入框。完成一次扫码绑定后，发给微信 bot 的文字或语音会自动追加到 Daily Notes 的 `## Journal` 区段，格式与侧边栏速记一致：
...
1. 使用 Obsidian 1.11.4 或更高版本的桌面端，并启用内置「Daily Notes」核心插件。
2. 打开 **设置 → Journal Partner → 微信采集**，点击「扫码绑定」。
3. 用手机微信扫码并确认；状态显示「已连接」后，直接给 bot 发消息即可。

实现边界：

- 连接直达腾讯官方 iLink（ClawBot）通道，没有作者运营的中转服务器，也不会调用 AI 改写正文。
- bot token 只保存在 Obsidian SecretStorage，不写入 vault 或插件 `data.json`。
- Obsidian 关闭期间由服务端游标补收；消息带原始时间时会写回对应日期，并用最近消息 ID 防止批次重放造成重复记录。
- 首版支持**文字**与**微信提供的语音转写**；图片、文件、视频、语音原声和每日提醒暂未接入。
- 通道仅在 Obsidian 桌面端打开时运行。账号能否扫码绑定取决于腾讯是否为该微信账号开放 ClawBot；这是腾讯侧限制，插件无法绕过。
